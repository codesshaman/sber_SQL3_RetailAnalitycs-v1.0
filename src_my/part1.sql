...
-- Procedure to import data from a CSV file for Personal_Information table
CREATE OR REPLACE PROCEDURE Import_Personal_Information_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Personal_Information (Customer_ID, Customer_Name, Customer_Surname, Customer_Primary_Email, Customer_Primary_Phone)
  FROM STDIN WITH (FORMAT CSV, DELIMITER p_delimiter);
END;
$$ LANGUAGE plpgsql;

-- Procedure to export data to a CSV file for Personal_Information table
CREATE OR REPLACE PROCEDURE Export_Personal_Information_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Personal_Information (Customer_ID, Customer_Name, Customer_Surname, Customer_Primary_Email, Customer_Primary_Phone)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
END;
$$ LANGUAGE plpgsql;

-- Procedure to import data from a CSV file for Cards table
CREATE OR REPLACE PROCEDURE Import_Cards_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for Cards table
CREATE OR REPLACE PROCEDURE Export_Cards_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Cards (Customer_Card_ID, Customer_ID)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for Transactions table
CREATE OR REPLACE PROCEDURE Import_Transactions_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for Transactions table
CREATE OR REPLACE PROCEDURE Export_Transactions_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Transactions (Transaction_ID, Customer_Card_ID, Transaction_Summ, Transaction_DateTime, Transaction_Store_ID)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for Checks table
CREATE OR REPLACE PROCEDURE Import_Checks_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for Checks table
CREATE OR REPLACE PROCEDURE Export_Checks_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Checks (Transaction_ID, SKU_ID, SKU_Amount, SKU_Summ, SKU_Summ_Paid, SKU_Discount)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for Product_Grid table
CREATE OR REPLACE PROCEDURE Import_Product_Grid_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Product_Grid (SKU_ID, SKU_Name, Group_ID, SKU_Purchase_Price, SKU_Retail_Price)
  FROM STDIN WITH (FORMAT CSV, DELIMITER p_delimiter);
END;
$$ LANGUAGE plpgsql;

-- Procedure to export data to a CSV file for Product_Grid table
CREATE OR REPLACE PROCEDURE Export_Product_Grid_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Product_Grid (SKU_ID, SKU_Name, Group_ID, SKU_Purchase_Price, SKU_Retail_Price)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for Stores table
CREATE OR REPLACE PROCEDURE Import_Stores_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for Stores table
CREATE OR REPLACE PROCEDURE Export_Stores_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Stores (Transaction_Store_ID, SKU_ID, SKU_Purchase_Price, SKU_Retail_Price)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for SKU_Group table
CREATE OR REPLACE PROCEDURE Import_SKU_Group_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for SKU_Group table
CREATE OR REPLACE PROCEDURE Export_SKU_Group_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY SKU_Group (Group_ID, Group_Name)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- Procedure to import data from a CSV file for Date_of_Analysis_Formation table
CREATE OR REPLACE PROCEDURE Import_Date_of_Analysis_Formation_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
...
-- Procedure to export data to a CSV file for Date_of_Analysis_Formation table
CREATE OR REPLACE PROCEDURE Export_Date_of_Analysis_Formation_CSV(p_delimiter VARCHAR)
AS $$
BEGIN
  COPY Date_of_Analysis_Formation (Analysis_Formation)
  TO STDOUT WITH (FORMAT CSV, DELIMITER p_delimiter);
...
-- CALL Import_Personal_Information_CSV(',');

-- CALL Export_Personal_Information_CSV(';');
